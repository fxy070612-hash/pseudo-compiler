...
setTimeout(function(){
  els.btnBook.onclick();
  const rows=[]; (function walk(n,d){ (n.children||[]).forEach(function(c){ if((c._cls||'').indexOf('book-it')===0) rows.push({row:c,texts:(function tt(x,o){o=o||[];(x.children||[]).forEach(function(y){if(y._text)o.push(y._text);tt(y,o);});return o;})(c)}); walk(c,d+1); }); })(els.bookList);
  console.log('书本行数='+rows.length);
  console.log('第一行内容='+JSON.stringify((rows[0]?rows[0].texts:[]).map(function(x){return String(x).slice(0,90);})));
  const cb=rows[0].row.children[0]; cb.checked=true; if(cb.onchange) cb.onchange();
