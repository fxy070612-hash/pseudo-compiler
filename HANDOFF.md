...
### 15.3 输出不符时的「出错行定位」（2026-09-24，用户当场提出）

用户原话：「我现在测试没通过，但是不知道是哪行代码出问题，应该有报错显示才对」。

问题：`alignPyToPseudo()` 只能在**有异常堆栈**时给出行号；纯「输出不符」（逻辑错，不崩溃）没有任何堆栈可映射，
界面只显示「期望 X，实际 Y」——学生仍然不知道哪一行错了。

做法：新增 `AI_LOCATE_SYS` + `locateProblem()`：把**带行号的伪代码**（`行号| 代码`）与第一个失败用例的
输入/期望/实际（以及报错文本）一起给 AI，让它返回最可能的 1~3 行 + 原因 + 建议；
行号会在应用侧校验（必须落在 1..总行数，去重、最多 3 行），然后标红并在结果卡片里显示「最可能出错的行：第 N 行」。
只在「没有异常堆栈可映射」时触发这一次额外调用（有堆栈就直接映射，省一次请求）。

**实测证据**（故意把 `best = 0` 写错，全负数组会答 0 而正确是 −2）：
```
实测检查（Python · 服务器真跑）· 通过 1/2
#2 【不符】期望 -2，实际 0　输入 {"A":[-5,-2,-3],"n":3}
最可能出错的行：第 3 行（已在源码里标红）
原因：第 3 行把 best 初始化为 0，全负数组时最大子段和是负数，返回 0 就错了
建议：best 应初始化为 A[1]
源码标红行数 = 1，行号标红 = 1
```
第 3 行正是 `best = 0` ✅ 定位准确。

### 15.4 编译失败的报错不再被折叠（2026-09-24，用户第二次反馈）

用户原话：「我现在还是编译未通过，然后没有报错，没有显示我的哪一步有问题」。

真正的原因：**编译器的诊断一直是对的**（`{level,line,col,msg}`，例如
`第 1 行：无法理解的语句` / `第 2 行：表达式有误，遇到 “:”`），但 `renderWorkspace()` 里
明细块 `<div class="comp-detail hidden">` **默认折叠**，状态行又只写「未通过编译」，要点「查看编译器提示」才看得到。

改法：
1. 失败时状态行直接带上第一条错误：`编译未通过（第 N 行：msg）`；
2. 失败时明细**强制展开**（`state.diagOpen = true`，且渲染条件写成 `(state.diagOpen || !ok)`），错误不该藏在折叠里；
3. 失败时把这些行**标红**（`setEditorMarks(errLines, 'compile')`），成功且当前是编译期标红时清除；
4. `setEditorMarks` 增加 `source` 参数（`'compile'` / `'check'`），编译期与实测期的标红互不覆盖。

实测（用用户的 Python 风格代码 `def select(A,i):`）：
状态行「编译未通过（第 1 行：无法理解的语句）」、明细 5 条且默认可见、源码与行号各标红 3 行。

### 15.5 语法错误的行号 + 「你在写 Python」提示（2026-09-24）

用户贴出一段 Python 风格的 SELECT 问「这个为什么报错」。查明两件事：

1. **诊断行号写死成 1**：`compile()` 的两个 catch 里都是 `{level:'error', line:1, col:1, msg:...}`，
   于是所有语法/词法错误都标在第 1 行，学生看不出错在哪。
   修法：从消息里的「（第 N 行）」把真实行号取出来（已实测：同一段代码由「第 1 行」变成**第 11 行**
   ——`medians.append(g[(len(g))//2])` 的 `.` 才是第一个卡住的地方）。
2. **没有告诉用户"这是 Python，不是伪代码"**：新增 `pythonMarkers()`，检测到 ≥2 个 Python 特征
   （def / import / elif / 行尾冒号 / x.y() / 列表推导 / len()sorted() 等）时，
   在编译失败块里直接给出一条红字说明 + 伪代码写法要点。
