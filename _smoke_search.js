...
setTimeout(function(){
  ...
  setTimeout(function(){
    // 先测搜索
    els.bookSearch.value='最短路'; if(els.bookSearch.oninput) els.bookSearch.oninput(); else if(els.bookSearch.onchange) els.bookSearch.onchange();
    console.log('   搜索前章节卡='+heads.length);
    els.btnBook.onclick();
    const heads2=(els.bookList.children||[]).filter(function(c){return (c._cls||'').indexOf('book-ch')>=0;});
    console.log('   搜索「最短路」后章节卡='+heads2.length+' 统计='+(els.bookStat._text||''));
